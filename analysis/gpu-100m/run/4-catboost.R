...
d_train <- fread("train-10m.csv", showProgress=FALSE, stringsAsFactors=TRUE)
...
d_train_test <- rbindlist(list(d_train, d_test))
...
dx_train_WUP <- catboost.load_pool(d_train[1:10000,1:p], label = d_train$dep_delayed_15min[1:10000])

rm(d_train, d_train_test)
gc()


params <- list(iterations = 1, depth = 10, learning_rate = 0.1,
   task_type = "GPU", devices = "0",
   verbose = 0)
cat(system.time({
  md <- catboost.train(learn_pool = dx_train_WUP, test_pool = NULL, params = params)
})[[3]]," ",sep="")
