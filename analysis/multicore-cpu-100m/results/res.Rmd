...
ggplot(d[tool=="xgboost" & size==100]) + geom_boxplot(aes(x = pcores, y = runtime), color = "blue") +
  coord_flip() + facet_wrap(~tool+size, scales = "free") + ylim(c(0,NA)) +
  theme_grey(base_size = 22)

ggplot(d[tool=="lightgbm" & size==100]) + geom_boxplot(aes(x = pcores, y = runtime), color = "red") +
  coord_flip() + facet_wrap(~tool+size, scales = "free") + ylim(c(0,NA)) +
  theme_grey(base_size = 22)
...
ddm <- dd[ncores %in% c(1,2,4,8,16,32),
    .(run_time = mean(runtime)),by=.(tool, size, ncores)][order(tool, size, ncores)]
...
ggplot(ddm) + geom_point(aes(x = as.factor(ncores), y = speedup)) + 
  facet_grid(tool~size) + theme(legend.position="none") +
  ylim(c(1,2))
